...
private  int counter =0;
    ...
    public ImagePaneExample(String dbimagePath,String outputPath,JLabel msg,JFrame frame) {
        ...
        EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    BufferedImage img = ImageIO.read(new File(dbimagePath));
                    ImagePane imgPane = new ImagePane();
                    imgPane.setImage(img);
                    imgPane.setRounded(true);
                    imgPane.setBorder(new EmptyBorder(20, 20, 20, 20));
                    
                    BufferedImage outputimg = ImageIO.read(new File(outputPath));
                    ImagePane outputimgPane = new ImagePane();
                    outputimgPane.setImage(outputimg);
                    outputimgPane.setRounded(true);
                    outputimgPane.setBorder(new EmptyBorder(20, 20, 20, 20));

                
                    JLabel dbOoutputlabel = new JLabel("Test");
        			dbOoutputlabel.setText("KTP Image");
        			System.out.println("counter"+counter);
        			if(counter<1) {
        			frame.add(dbOoutputlabel);
                    frame.add(imgPane);
        			}
                    
                    JLabel capturedlabel = new JLabel("Test");
                    capturedlabel.setText("Webcam Captured Image");
                    if(counter<1) {
        			frame.add(capturedlabel);
                    frame.add(outputimgPane);
                    }
